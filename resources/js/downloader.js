...
const els = {
      ...
      advToggle: el('advToggle'), advPanel: el('advPanel'),
      cookiesText: el('cookiesText'), cookiesFile: el('cookiesFile'),
};
...
async function onSubmit(e) {
      ...
      const cookies = (els.cookiesText?.value || '').trim();
      if (cookies.length > 102400) return fail('That cookies file is too large (max 100 KB).');
...
// Advanced cookies panel
els.advToggle?.addEventListener('click', () => els.advPanel.classList.toggle('hidden'));
els.cookiesFile?.addEventListener('change', () => {
      const file = els.cookiesFile.files?.[0];
      if (!file) return;
      if (file.size > 102400) { fail('That cookies file is too large (max 100 KB).'); return; }
      const reader = new FileReader();
      reader.onload = () => { els.cookiesText.value = reader.result; };
